...
def ready():
    global s_game
    ready.configure(text = "游戏开始",command = '')
    ganmePort = 50010    # 游戏端口为50010
    s = socket.socket( socket.AF_INET, socket.SOCK_STREAM )
    s.connect((IP, ganmePort))
    s.send("ready".encode())
    def startgameRecv():
        while 1:
            data5  = s.recv(1024).decode()
            listbox.insert(tkinter.END, data5, 'red')


    stgameRecv = threading.Thread(target= startgameRecv)
    stgameSend = threading.Thread(target= sendtogame)
    button.configure(command = sendtogame)


    # 给全局 s_game 变量赋值
    s_game = socket.socket( socket.AF_INET, socket.SOCK_STREAM )
    s_game.connect((IP, ganmePort))
    stgameRecv.start()
    stgameSend.start()
...
# 创建准备按钮
ready = tkinter.Button(root, text='准备游戏', command=ready)
ready.place(x=185, y=320, width=60, height=30)
